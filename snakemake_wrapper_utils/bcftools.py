...
def get_bcftools_opts(snakemake):
    ...
    ###############
    if "--threads" in extra:
        sys.exit("You have specified number of threads in params.extra; please use only `threads`.")
    bcf_opts += "" if snakemake.threads <= 1 else "--threads {}".format(snakemake.threads - 1)
    ...
    if "-O" in extra or "--output-type" in extra:
        sys.exit("You have specified output format in params.extra; this is automatically infered from output file extension.")
    ...
    if "-m" in extra or "--max-mem" in extra:
        sys.exit("You have provided `-m/--max-mem` in params.extra; please only use resources.mem_mb and set params.bcftools_use_mem.")
    ...
    if "-T" in extra or "--temp-dir" in extra:
        sys.exit("You have provided `-T/--temp-dir` in params.extra; please provide a temp dir as output.bcftools_temp.")
